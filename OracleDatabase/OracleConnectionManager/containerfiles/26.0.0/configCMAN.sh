...
get_dbhost_details()
{
    ...
    db_hostdetail_values="$(strip_assignment_prefix "${DB_HOSTDETAILS}")"
...
reload_cman()
{
    "$DB_HOME/bin/cmctl" reload -c "CMAN_${PUBLIC_HOSTNAME}.${DOMAIN}"
}

start_cman()
{
    export ORACLE_HOME="$DB_HOME"
    "$DB_HOME/bin/cmctl" startup -c "CMAN_${PUBLIC_HOSTNAME}.${DOMAIN}"
}
...
{
    export ORACLE_HOME="$DB_HOME"
    "$DB_HOME/bin/cmctl" show service -c "CMAN_${PUBLIC_HOSTNAME}.${DOMAIN}"
...
parse_delete_rule_details()
{
    ...
    del_rule_details="$(strip_assignment_prefix "${RULEDETAILS}")"
    reset_rule_vars
